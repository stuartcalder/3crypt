...
#if 0
    void CBC_V2_encrypt(Arg_Map_t const &opt_arg_pairs)
    {
        using namespace std;
...
#endif
        // Copy the header into the file, field at a time, advancing the pointer
        {
...
#if 0
    void CBC_V2_decrypt(Arg_Map_t const &opt_arg_pairs)
    {
        using namespace std;
        ...
        in += sizeof(header);
#endif
        // Copy all the fields of CBC_V2_Header_t into the header struct
        {
            ...
            memcpy( header.cbc_iv,      in, sizeof(header.cbv_iv)       );
